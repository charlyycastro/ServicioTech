...
class ServiceOrder(models.Model):
    ...
    # Campo de texto antiguo (lo dejamos por compatibilidad con órdenes viejas)
    contacto_nombre = models.CharField("Contacto Interno (Texto)", max_length=200, blank=True, null=True)

    # --- NUEVO CAMPO: VISOR / CONTACTO VENTAS ---
    # Esto permite seleccionar un usuario del sistema y usar su firma guardada
    ...
    def save(self, *args, **kwargs):
        if not self.folio:
            ...
            prefix = f"OS-{today}"
            # Tomamos la inicial del ingeniero o 'X' si no hay
