...
class OrderListView(LoginRequiredMixin, ListView):          # ← protege "/"
    login_url = 'login'
...
class OrderDetailView(LoginRequiredMixin, DetailView):      # ← protege detalle
    login_url = 'login'
    redirect_field_name = 'next'
    model = ServiceOrder
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            ...
            if order.cliente_email:
                ...
                email = EmailMessage(subject=f"Orden de Servicio {order.folio}", body=html, from_email=None, to=[order.cliente_email])
                email.content_subtype = 'html'
                try:
                    if order.firma: email.attach_file(order.firma.path)
                except Exception:
                    pass
                try:
